...
export const getCountriesAction = () => {
    return async function (dispatch) {
        const countriesData = (await axios.get(`${urlBase}/countries`)).data;
        dispatch({
            type: GET_COUNTRIES,
...
export const getCountryByIdAction = (id) => {
    return async function (dispatch) {
        const countryData = (await axios.get(`${urlBase}/countries/${id}`)).data;
        dispatch({
            type: GET_COUNTRY,
            payload: countryData,
        });
...
export function searchByName(name) {
    return async function (dispatch) {
        try {
            const countrySearchData = (await axios.get(`${urlBase}/countries?name=${name}`)).data;
...
export const getActivitiesAction = () => {
    return async function (dispatch) {
        const activitiesData = (await axios.get(`${urlBase}/activities`)).data;
        dispatch({
            type: GET_ACTIVITIES,
